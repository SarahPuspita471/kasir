...
<style>
  ...
  /* @apply di sini opsional (akan bekerja kalau Tailwind dikompilasi) */
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
  ...
  <aside class="card p-4 sm:p-6 flex flex-col h-full anim-in">
    ...
    <div class="mt-3">
      ...
      <input id="customerName" type="text" class="mt-1 w-full rounded-md border-gray-300 px-3 py-2" placeholder="cth. Budi">
    </div>
      ...
      <div class="border-t pt-3 space-y-2 text-sm">
        ...
        <div>
          <div class="flex items-center gap-2">
            ...
            <button id="applyVoucher" class="px-3 py-1.5 rounded-md bg-indigo-600 text-white text-xs">Terapkan</button>
...
<div id="payModal" class="hidden fixed inset-0 z-50 bg-black/40 flex items-end md:items-center justify-center p-4">
  <div class="bg-white w-full max-w-lg rounded-2xl shadow-xl p-6 anim-in">
    ...
    <div class="mt-3 space-y-2 text-sm">
      ...
      <div class="flex justify-between text-lg font-semibold"><span>Total</span><span id="payTotal">Rp 0</span></div>
    </div>

    <div class="mt-4">
      <label class="text-sm text-gray-600">Metode</label>
      <div class="mt-2 grid grid-cols-3 gap-2">
        <button type="button" class="methodBtn rounded-lg border py-2 hover:bg-gray-50" data-method="cash">Tunai</button>
        <button type="button" class="methodBtn rounded-lg border py-2 hover:bg-gray-50" data-method="noncash">Non-Tunai</button>
        <button type="button" class="methodBtn rounded-lg border py-2 hover:bg-gray-50" data-method="mixed">Campuran</button>
      </div>
    </div>
...
function setQty(kode, qty){
  ...
  qty = Math.max(0, Math.min(qty, it.stok));
...
function refreshTotals(){
  ...
  const total    = Math.max(0, subtotal - AUTO_DISC - (VOUCHER.amount||0));

  const subEl = el('#subtotalText'); if (subEl) subEl.textContent = rupiah(subtotal);
  const adEl  = el('#autoDiscText'); if (adEl)  adEl.textContent  = rupiah(AUTO_DISC);
  const vdEl  = el('#voucherDiscText'); if (vdEl) vdEl.textContent = rupiah(VOUCHER.amount||0);
  const totEl = el('#totalText'); if (totEl) totEl.textContent = rupiah(total);

  const paySub = el('#paySubtotal'); if (paySub) paySub.textContent = rupiah(subtotal);
  const payDis = el('#payDisc');     if (payDis) payDis.textContent = rupiah(AUTO_DISC + (VOUCHER.amount||0));
  const payTot = el('#payTotal');    if (payTot) payTot.textContent = rupiah(total);
}
...
function renderCart(){
  ...
  // hitung auto discount dari server, lalu refresh angka
  previewAutoDiscount();
}
...
if (cashInput){
  cashInput.addEventListener('input', ()=>{
    ...
    const subtotal = CART.reduce((s,i)=> s + (i.harga*i.qty), 0);
    const total = Math.max(0, subtotal - AUTO_DISC - (VOUCHER.amount||0));
    const kt = el('#kembalianText'); if (kt) kt.textContent = rupiah(Math.max(0, paid - total));
  });
}
